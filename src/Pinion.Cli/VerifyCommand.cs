...
internal static class VerifyCommand
{
        ...
        string? since, string? source, bool verbose, CancellationToken ct)
    {
        ...
        try
        {
            ...
            WarnIfGeneratedByNewerPinion(Path.Combine(Path.GetDirectoryName(Path.GetFullPath(testProject))!, subdir));
    ...
    /// <summary>Warn if the generated tests were stamped by a NEWER Pinion format than this build supports —
    /// so the user updates the tool rather than chasing phantom "behavior changed" diffs from a format skew.</summary>
    private static void WarnIfGeneratedByNewerPinion(string outDir)
    {
        ...
        if (newest > CharacterizationFormat.Version)
            Console.Error.WriteLine(
                $"note: these characterization tests were generated by a newer Pinion (format v{newest}; this build " +
                $"understands v{CharacterizationFormat.Version}). Update Pinion (`dotnet tool update -g Pinion`) so verify " +
                "matches how they were captured.");
    }
